...
namespace P1
{
    class Program
    {
        static readonly string root = @"C:\Users\Korisnik\Desktop\faks prez\sistemsko\sistemskoP1";   
        ...
        static void ObradiZahtev(object state)
        {
            ...
            if (kes.TryGetValue(cacheKey, out string? htmltextresponse))
            {
                ...
                context.Response.OutputStream.Write(bafer, 0, bafer.Length);
            ...
            else
            {
                ...
                htmltext = VratiHtml(trazeneReci);
        ...
        static string VratiHtml(string[] reci)
        {
            ...
            foreach (string rec in reci)
            {
                ...
                List<Fajl> fajlovi = PronadjiReci(rec, root);
        ...
        //funkcija pronalazi trazenu rec u fajlovima u kojima postoji, i stampa odgovarajuce poruke u konzoli
        static List<Fajl> PronadjiReci(string rec, string root)
        {
            ...
            foreach (string filePath in Directory.EnumerateFiles(rootDir, "*.txt", SearchOption.TopDirectoryOnly))  
            {
                ...
                string tekstfajla = File.ReadAllText(filePath);
